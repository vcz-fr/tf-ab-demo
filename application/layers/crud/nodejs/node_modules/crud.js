...
exports.createItem = Item =>
    DynamoDB.put({ TableName, Item }, responseHandler('CREATE')).promise();

exports.readItem = async Key => {
    const KeyConditionExpression = `${
        Object.entries(Key)
            .map(([field, _]) =>
                `${field}=${normalizeField(field)}`).join(' and ')
        }`;

    const ExpressionAttributeValues = Object.assign(...Object.entries(Key)
        .map(([field, value]) =>
            ({ [normalizeField(field)]: value })));

    const response = await DynamoDB.query({ TableName, KeyConditionExpression, ExpressionAttributeValues }).promise();
